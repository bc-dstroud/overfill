...
cols_to_drop = ['MIN_DATE', 'MAX_DATE', 'FINAL_NEEDED', 'JOB_OVERFILL' ]
...
df2['overfill_rate']=df2.apply(lambda x: x['total_applicants']/(x['NEEDED']+x['OVERFILL']) if (x['NEEDED']+x['OVERFILL'])>0 else -1, axis = 1)
...
df3 = df2[df2['LEAD_TIME_HOURS'] == df2['total_lead_time']]
...
df5 = df5[(df5['LEAD_TIME_HOURS']!=0)&(df5['total_applicants']>=0)&(df5['NEEDED']>1)&(df5['FILL_RATE']< 4)]
df5.describe()

# COMMAND ----------

df6 = df5[df5['LEAD_TIME_HOURS'] == df5['total_lead_time']]
df6[df6['current_fill_rate']==0].shape
# df6.shape
df6

# COMMAND ----------

display(sdf[sdf["JOB_ID"]==323450])

# COMMAND ----------

df = spark.sql(""" select * from feature_store.dev.job_applicant_tracker where job_id = 323450""")
display(df)
...
sns.histplot(data=x2, x='prediction',palette="Spectral",
   alpha=.5, linewidth=0,bins=10, multiple='stack')
plt.show()
...
# Print out model metrics
print("Comparing Fill Rate for jobs before SNCs:")
print("  RMSE: %s" % rmse)
print("  MAE: %s" % mae)
print("  R2: %s" % r2)
print("  MAPE: %s" % mape)
...
# Print out model metrics
print("Comparing Fill Rate for jobs at job start:")
...
cols_to_plot2 = ['WAGE_DELTA_PERCENT', 'JOB_TYPE', 'JOB_WAGE', 'JOB_TITLE', 'JOB_NEEDED_LAST_COUNT', 'JOB_SHIFTS','FILL_RATE' ]
...
sns.histplot(data=x3, x='FILL_RATE',palette="Spectral",
   alpha=.5, linewidth=0,bins=10, multiple='stack')
plt.show()

# COMMAND ----------

sns.scatterplot(data=x2, x='LEAD_TIME_HOURS',y='FILL_RATE',
   alpha=.1)
plt.show()

# COMMAND ----------

sns.scatterplot(data=x2, x='current_fill_rate',y='FILL_RATE',
   alpha=.1)
plt.show()
  ...
  if np.random.rand() < .05:
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers+lines",
            marker_color='mediumpurple', opacity=0.08, 
            showlegend=False)
    )
fig.update_layout(
  height = 1000,

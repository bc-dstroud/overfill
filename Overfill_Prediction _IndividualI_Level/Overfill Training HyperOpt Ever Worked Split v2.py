...
sdf = sdf.filter((sdf.NEEDED >0)&(sdf.COMPANY_ORIGIN == 'BC')&(sdf.JOB_START_TIME>=sdf.JOB_CREATED_AT))
...
# write_spark_table_to_databricks_schema(optimize_spark(sdf), 'overfill_training_labels_v2', 'bluecrew.ml')
...
cols_to_drop = [ 'JOB_OVERFILL', 'INVITED_WORKER_COUNT', 'SEGMENT_INDEX', 'NEEDED', 'JOB_ID','USER_ID', 'COUNTY','target_var', 'application_status', 'successful_application_count', 'max_successful_app_start', 'min_successful_app_end', 'max_successful_app_end', 'JOB_TITLE', 'COMPANY_ORIGIN']
# 'JOB_STATUS', 'JOB_STATUS_ENUM', 'POSITION_ID', 'COMPANY_ID', 'SCHEDULE_ID', 'JOB_CITY','JOB_STATE', 
...
df2 = df[(df['target_var'] !='Early Cancel')&(df['target_var'] != 'SNC')&(df['JOB_CREATED_AT']>='2023-01-01')& (df['POSTING_LEAD_TIME_DAYS']>0)&(df['commute_distance']<=70)]
# &(df['JOB_STATUS']!='Cancelled')
...
registry_model_name = "bluecrew.ml.overfill_prior_worked_2"
...
# # Define the model name for the registry
registry_model_name2 = "bluecrew.ml.overfill_never_worked_2"

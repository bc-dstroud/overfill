...
model2 = mlflow.sklearn.load_model(model_uri=f"models:/{registry_model_name}/4")
...
model_info = mlflow.models.get_model_info(f"models:/{registry_model_name}/4")
...
predictions = pd.Series(final_model.predict_proba(X_test)[:,1])
...
sns.kdeplot(
   data=x2, x='predicted_probability', hue="ever_worked",
   fill=True, common_norm=False, palette="crest",
   alpha=.5, linewidth=0,
)
...
sns.scatterplot(data=job_modeling, x='Work',y='predicted_probability',
   alpha=.1)
plt.show()

# COMMAND ----------

sns.scatterplot(data=x2, x='WORKED_RATIO_SNC',y='predicted_probability',
   alpha=.05, hue='Work')
plt.show()

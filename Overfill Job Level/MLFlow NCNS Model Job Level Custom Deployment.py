...
cols_to_drop = ['SUCCESSFULLY_WORKED', 'TOTAL_SUCCESSFUL_SIGN_UPS', 'JOB_STATUS_ENUM', 'JOB_STATUS', 'JOB_OVERFILL', 'INVITED_WORKER_COUNT', 'SEGMENT_INDEX', 'NEEDED', 'JOB_NEEDED_LAST_COUNT', 'SIGN_UP_JOIN_COUNT', 'START_TIME', 'POSITION_ID', 'COMPANY_ID', 'COUNTY_JOB_TYPE_TITLE_AVG_WAGE', 'day_of_week']
...
for col in X_valid.columns:
  print(col, " Data type: ", X_valid[col].dtype)

# COMMAND ----------

import mlflow
logged_model = 'runs:/11cfb24a4bfb4d069196eaa3f88f5f2f/model'
...
custom_wrapper = CustomModelWrapper(model)
...
test_df = pd.DataFrame(X_valid[:1], columns=wine_data.feature_names)
...
overfill_added[overfill_added['JOB_ID']==309808]

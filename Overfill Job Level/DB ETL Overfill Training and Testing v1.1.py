...
WITH tmp_first_shift AS (
    ...
    FROM bluecrew_prod.bluecrew.TIME_SEGMENTS_ABSOLUTE tsa
    LEFT JOIN bluecrew_prod.bluecrew.SCHEDULE_WORK_REQUESTS s
...
tmp_first_shift_full AS (
    -- get all columns for first shift and job information
        ...
        LEFT JOIN bluecrew_prod.bluecrew.TIME_SEGMENTS_ABSOLUTE tsa
...
--                 AND JOB_STATUS_ENUM < 6 -- active jobs only
    ...
    LEFT JOIN bluecrew_snowflake.dm.DM_CM_JOB_APPLIED_HISTORY jah
...
tmp_first_shift_sign_up_count AS (
    ...
    FROM bluecrew_snowflake.dm.dm_cm_hours_worked
...
from jacw
left join bluecrew_snowflake.dm.dm_companies c
...
cols_to_drop = ['SUCCESSFULLY_WORKED', 'TOTAL_SUCCESSFUL_SIGN_UPS', 'JOB_STATUS_ENUM', 'JOB_STATUS', 'JOB_OVERFILL', 'INVITED_WORKER_COUNT', 'SEGMENT_INDEX', 'NEEDED', 'SIGN_UP_JOIN_COUNT', 'POSITION_ID', 'COMPANY_ID', 'COUNTY_JOB_TYPE_TITLE_AVG_WAGE']

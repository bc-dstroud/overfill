...
WITH tmp_first_shift AS (
    ...
    WHERE tsa.ACTIVE = TRUE
        ...
        AND tsa.CREATED_AT < TIMESTAMP(tsa.START_TIME)
...
tmp_first_shift_full AS (
    -- get all columns for first shift and job information
        SELECT
            ...
            tsa.SEGMENT_INDEX,
            j.POSITION_ID,
            TIMESTAMP(tsa.START_TIME) AS START_TIME,
            TIMESTAMPADD(HOUR, -(tsa.START_TIME_OFFSET/60), TIMESTAMP(tsa.START_TIME)) AS START_TIME_LOCAL,
            TIMESTAMP(tsa.END_TIME) AS END_TIME,
            TIMESTAMPADD(HOUR, -(tsa.END_TIME_OFFSET/60), TIMESTAMP(tsa.END_TIME)) AS END_TIME_LOCAL,
            fs.SCHEDULE_ID,
            j.JOB_CREATED_AT,
            j.JOB_TYPE,
            j.JOB_TITLE,
        ...
        LEFT JOIN BLUECREW.MYSQL_BLUECREW.TIME_SEGMENTS_ABSOLUTE tsa
            ...
            AND TIMESTAMP(tsa.START_TIME) = timestamp(fs.FIRST_SHIFT_START_TIME)
        ...
        LEFT JOIN BLUECREW.DM.DM_JOB_NEEDED_HISTORY nh
            ...
            AND TIMESTAMP(START_TIME) BETWEEN START_DATE AND END_DATE
        INNER JOIN BLUECREW.DM.DM_JOBS j
            ON j.JOB_ID = fs.JOB_ID
--                 AND JOB_STATUS_ENUM < 6 -- active jobs only
        WHERE YEAR(TIMESTAMP(START_TIME)) >= 2020
            AND timestamp(START_TIME) <= DATEADD(DAY, 28, CURRENT_DATE())
...
select jacw.*, c.COMPANY_ORIGIN from jacw
...
df2 = df[(df['Work'].isnull() == False) & (df['Work']<=1)&(df['JOB_NEEDED_ORIGINAL_COUNT']>=5)&(df['POSTING_LEAD_TIME_DAYS']>0)&(df['NEEDED']>=3)&(df['TOTAL_SUCCESSFUL_SIGN_UPS']>=3)&(df['COMPANY_ORIGIN']!="EB")]
...
small_jobs = df[df['JOB_NEEDED_ORIGINAL_COUNT']<=4]
df.shape[0]

# COMMAND ----------

sns.histplot(data = small_jobs['JOB_NEEDED_ORIGINAL_COUNT'], kde = False)
plt.show()
...
c = pd.concat([a,b])
c['lower_guess']=(c['Predicted_Show_Up_Rate'] + c['lowq']).clip(lower = 0) #can't have negative numbers
...
(rmse, mae, r2) = eval_metrics(overfill_added_test['Actual_Show_Up_Rate'], overfill_added_test['PAST_PREDICTED_Show_Up_Rate'])
...
(rmse, mae, r2) = eval_metrics(overfill_added2['Actual_Show_Up_Rate'], overfill_added2['PAST_PREDICTED_Show_Up_Rate'])

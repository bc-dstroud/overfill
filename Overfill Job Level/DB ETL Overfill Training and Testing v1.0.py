...
tmp_first_shift_full AS (
    ...
    LEFT JOIN bc_foreign_snowflake.dm.DM_CM_JOB_APPLIED_HISTORY jah
...
# MAGIC %sql
# MAGIC drop table if exists bluecrew.ml.overfill_training_daily
...
write_spark_table_to_databricks_schema(sdf, 'overfill_training_daily', 'bluecrew.ml')

# COMMAND ----------

sdf = spark.read.format("delta").table('bluecrew.ml.overfill_training_daily')

# COMMAND ----------

sdf = sdf.filter((sdf.NEEDED >=3)&(sdf.TOTAL_SUCCESSFUL_SIGN_UPS>=3)&(sdf.COMPANY_ORIGIN == 'BC')&(sdf.JOB_NEEDED_ORIGINAL_COUNT>=5)&(sdf.TOTAL_SUCCESSFUL_SIGN_UPS>=sdf.SUCCESSFULLY_WORKED)&(sdf.POSTING_LEAD_TIME_DAYS>0)|((sdf.START_TIME_LOCAL>end_date)&(sdf.NEEDED>0)&(sdf.JOB_STATUS!='Cancelled')))
print((sdf.count(), len(sdf.columns)))
...
y = df5.loc[df5['START_TIME_LOCAL']<=training_end_date,'Work']
...
X = X[X['START_TIME_LOCAL']<=training_end_date]
...
df = df.withColumn('JOB_ID', df['JOB_ID'].cast('long')).withColumn('JOB_TYPE_TITLE_COUNT', df['JOB_TYPE_TITLE_COUNT'].cast('double')).withColumn('TOTAL_JOB_COUNT', df['TOTAL_JOB_COUNT'].cast('double'))
...
inference_df = df[df['Dataset']=='Future']
inference_df.write.format("snowflake").options(**options).mode("overwrite").option("dbtable", 'OVERFILL_TEST').save()
training_df = df[df['Dataset']!='Future']
write_spark_table_to_databricks_schema(inference_df, 'overfill_inference', 'bluecrew.ml', mode = 'append')
write_spark_table_to_databricks_schema(training_df, 'overfill_training', 'bluecrew.ml', mode = 'append')
